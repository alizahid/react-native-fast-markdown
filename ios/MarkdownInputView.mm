...
- (void)updateProps:(const Props::Shared &)props
  ...
  // UITextView doesn't have placeholder natively — would need custom implementation
  ...
  // Cursor color
  if (!newProps.cursorColor.empty()) {
    NSString *colorStr =
        [NSString stringWithUTF8String:newProps.cursorColor.c_str()];
    // Apply cursor color if valid
  }
...
- (void)toggleFormatting:(NSString *)marker {
  ...
  if (selectedRange.length == 0) {
    ...
    NSString *selectedText =
        [text substringWithRange:selectedRange];
...
- (void)removeLink {
  // Simple implementation: remove markdown link syntax around cursor
  // A more complete implementation would parse the link at cursor position
  [self applyMarkdownFormatting];
  [self emitChangeEvents];
}
...
- (void)applyMarkdownFormatting {
  ...
  // Parse markdown and apply attributed formatting
  ...
  // The rendered output and input text may differ in length due to
  // paragraph separators. For the editor, we apply spans to the original text.
  // This is a simplified approach; a production implementation would track
...
- (void)detectFormattingState {
  ...
  // Simple heuristic: check if cursor is inside formatting markers
  _isBold = [self isInsideMarker:@"**" inText:text atPosition:range.location];
  _isItalic = [self isInsideMarker:@"*" inText:text atPosition:range.location] && !_isBold;
  _isStrikethrough = [self isInsideMarker:@"~~" inText:text atPosition:range.location];
  _isCode = [self isInsideMarker:@"`" inText:text atPosition:range.location];
...
- (BOOL)isInsideMarker:(NSString *)marker
  ...
  // Cursor is between opening and closing markers
...
- (void)emitChangeEvents {
  ...
  // onChangeMarkdown (same as text for now — editor stores raw markdown)
